document.addEventListener("DOMContentLoaded", function () {
  ...
  const otherType = document.getElementById("other-type");const incidentDate = document.getElementById("incident-date");
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (!termsAgreement.checked) {
      alert("Um die Meldung einzureichen, müssen Sie den Nutzungsbedingungen zustimmen.");
      return;
    }
    ...
    emailjs.send("service_766sama", "template_pjl3ijd", formData).then(
      ...
      function (error) {
        alert("Beim Senden Ihrer Meldung ist ein Fehler aufgetreten. Bitte versuchen Sie es erneut.");
      }
    );
  });
  ...
  function validateStep1() {
    ...
    if (incidentType.value === "other" && !otherType.value.trim()) {
      markInvalid(otherType, "Bitte geben Sie die Art des Ereignisses an");
      isValid = false;
    } else if (incidentType.value === "other") {
      markValid(otherType);
    }
    if (!recipient.value === "other" && !otherType.value.trim()) {
      markInvalid(recipient, "Bitte wählen Sie einen Empfänger aus");
      isValid = false;
    } else if (recipient.value === "other") {
      markValid(recipient);
    }
    return isValid;
  }
  ...
  function validateStep2() {
    ...
    if (!incidentLocation.value.trim()) {
      markInvalid(incidentLocation, "Bitte wählen Sie einen Ort");
      isValid = false;
    } else {
      markValid(incidentLocation);
    }
    if (!incidentDescription.value.trim()) {
      markInvalid(incidentDescription, "Bitte geben Sie eine Beschreibung des Vofalls an");
      isValid = false;
    } else {
